...
if (process.argv.includes("--write-sql-seed")) {
  const fs = await import("fs");
  const path = await import("path");
  const { fileURLToPath } = await import("url");
  const dir = path.dirname(fileURLToPath(import.meta.url));
  const values = entries.map((e, i) => {
    const storage_path = e.legacyFile;
    const label = e.label.replace(/'/g, "''");
    return `  ('${e.id}', '${label}', '${e.style}', '${e.category}', '${storage_path}', ${i}, true)`;
  });
  const sql = `-- Regenerated seed for svg_clothes (run after table exists)
...
on conflict (id) do update set
  ...
  updated_at = now();
`;
  fs.writeFileSync(path.join(dir, "../supabase/migrations/012_svg_clothes_seed_refresh.sql"), sql);
  console.error("Wrote supabase/migrations/012_svg_clothes_seed_refresh.sql");
} else {
